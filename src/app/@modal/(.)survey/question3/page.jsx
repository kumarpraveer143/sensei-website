...
const Survey3 = () => {
    ...
    const options = [
        {
            label: "They talk about how they feel and try to calm down.",
            skills: ["Self-awareness"]
        },
        {
            label: "They cry or get angry quickly.",
            skills: ["Coping with Emotions"]
        },
        {
            label: "They don't say anything and go quiet.",
    ...
    const handleNext = () => {
        if (selectedSkills.length > 0) {
            addSkillsForSurvey('survey3', selectedSkills);
            router.push('/survey/question4');
        } else {
            ...
            <div className="w-full max-w-[848px] max-h-[95vh] h-auto bg-white rounded-2xl shadow-xl relative px-4 sm:px-6 py-6 sm:py-8 overflow-auto">
                ...
                <div className="absolute top-20 left-1/2 -translate-x-1/2 flex items-center justify-center gap-2 w-full max-w-xs sm:max-w-sm md:max-w-md">
                    {[...Array(7)].map((_, index) => (
                        <div
                            key={index}
                            className={`h-1 flex-1 max-w-[40px] sm:max-w-[48px] md:max-w-[56px] rounded-full ${index <= 2 ? 'bg-orange-400' : 'bg-gray-200'
                                }`}
                        />
                    ))}
                </div>
                ...
                <div className="flex flex-col items-center text-center w-full max-w-[90%] sm:max-w-lg lg:max-w-xl mx-auto pt-20 sm:pt-24 pb-8">
                    ...
                    <h2 className="text-1xl sm:text-1xl lg:text-2xl font-bold text-[#2b3a78] mb-8 sm:mb-12 leading-snug">
                        If your child loses a game or is scolded, how do they react?
                    </h2>
                    ...
                    <button
                        ...
                        className={`mt-10 sm:mt-12 bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 px-10 sm:px-12 rounded-xl shadow-md transition text-base sm:text-lg lg:text-xl ${!selectedOption ? 'opacity-50 cursor-not-allowed' : ''
                            }`}
                    >
                        Next
                    </button>
