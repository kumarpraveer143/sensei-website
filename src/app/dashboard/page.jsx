...
const UserDashboard = () => {
  ...
  const fetchSubjectData = async () => {
    ...
    try {
      ...
      try {
        ...
        if (
          res?.data?.pricingPlan?.subjects &&
          res.data.pricingPlan.subjects.length > 0
        ) {
          console.log(
            "Setting user-specific subjects from pricingPlan:",
            res.data.pricingPlan.subjects,
          );
      ...
      if (!hasUserData) {
        try {
          ...
          console.error(
            "Error fetching general subjects:",
  ...
  useEffect(() => {
    const surveyDataNeedAttention = localStorage.getItem(
  ...
  return (
    <>
      <div className="container relative mx-auto my-4 mb-4 flex h-fit w-fit flex-col items-center gap-10 from-gray-200 to-white p-4 py-10 sm:py-20 md:my-8">
        <Background1 />
          ...
          <div className="flex flex-col items-start gap-1 py-4">
            ...
            {surveyDataNeedAttention.length > 0 ? (
              <div className="mt-6 flex flex-col">
                <div className="justify-start font-Nunito text-xl font-bold uppercase text-amber-500">
                  Personalized report
                </div>
                <div className="justify-start font-Nunito text-xl font-extrabold leading-loose text-black">
                  Life-skills your child shows:
                </div>
                <div className="inline-flex items-center justify-start gap-24">
                  <div className="flex items-center justify-start gap-1">
                    <div className="h-3 w-3 rounded-2xl bg-teal-600" />
                    <div className="justify-start font-['Nunito'] text-base font-bold leading-loose text-stone-500">
                      Strong
                    </div>
                  </div>
                  <div className="flex w-36 items-center justify-start gap-1">
                    <div className="h-3 w-3 rounded-2xl bg-red-500" />
                    <div className="justify-start font-['Nunito'] text-base font-bold leading-loose text-stone-500">
                      Need Attention
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-start gap-8">
                  <div className="relative">
                    <div className="absolute bottom-0 left-0 top-0 w-2 rounded-2xl bg-teal-600" />
                    <div className="ml-6">
                      {surveyDataStrong.map((skill, i) => (
                        <p
                          key={i}
                          className="font-Nunito text-base leading-loose"
                        >
                          {skill}
                        </p>
                      ))}
                  ...
                  <div className="relative">
                    <div className="absolute bottom-0 left-0 top-0 w-2 rounded-2xl bg-red-500" />
                    <div className="ml-6">
                      {surveyDataNeedAttention.map((skill, i) => {
                        return (
                          <p
                            key={i}
                            className="font-Nunito text-base leading-loose"
